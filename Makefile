...
# operating system:  Linux 4.6.3-1-ARCH x86_64 [ELF] (4.6.3)
# libc:	(6.23.0)
# binutils:	(226)
...
install:: xphoon
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTPGMFLAGS)  xphoon $(DESTDIR)$(BINDIR)/xphoon

all:: xphoon.$(MANNEWSUFFIX)

xphoon.$(MANNEWSUFFIX): xphoon.$(MANSRCSUFFIX)
	$(RM) $@
	if test -z "$(PREPROCESSMANPAGES)" ; then \
	   cd `dirname xphoon` && \
	   $(LN) `basename xphoon.$(MANSRCSUFFIX)` `basename $@`; \
	else \
	      	     $(RAWCPP)  $(MANDEFS) $(EXTRAMANDEFS) \
	     < xphoon.$(MANSRCSUFFIX) | sed -e '/^#  *[0-9][0-9]*  *.*$$/d' 			-e '/^#line  *[0-9][0-9]*  *.*$$/d' 			-e '/^[ 	]*XCOMM$$/s/XCOMM/#/' 			-e '/^[ 	]*XCOMM[^a-zA-Z0-9_]/s/XCOMM/#/' 			-e '/^[ 	]*XHASH/s/XHASH/#/' 			-e '/\@\@$$/s/\@\@$$/\\/' >$@; \
	fi

cleandir::
	$(RM) xphoon.$(MANNEWSUFFIX)

install.man:: xphoon.$(MANNEWSUFFIX)
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTMANFLAGS) xphoon.$(MANNEWSUFFIX) $(DESTDIR)$(MANDIR)/xphoon.$(MANSUFFIX)
